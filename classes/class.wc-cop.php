...
class WC_Gateway_Cash_on_pickup extends WC_Payment_Gateway {
	...
	/**
	 * The identifiers a chosen shipping rate can be matched against.
	 *
	 * The first is the rate's own id, which is its real identity. The second is that id with any
	 * suffix removed, rebuilt the way WC_Shipping_Method::get_rate_id() does it - note that it
	 * leaves the instance id out when the method has none, rather than writing ":0".
	 *
	 * That detail matters. The Checkout block's pickup_location has no zone instance, so its
	 * instance id is 0, yet it emits one rate per configured location as "pickup_location:0",
	 * "pickup_location:1" and so on. Appending ":0" here would manufacture an id identical to the
	 * first location's real rate id, and a setting naming one location would then match a
	 * customer who chose a different one.
	 *
	 * @param  string $method_id   Shipping method id.
	 * @param  int    $instance_id Zone instance id, 0 when the method has no zone instance.
	 * @param  string $rate_id     The rate's own id, where one exists.
	 * @return string[]
	 */
	private function get_rate_identifiers( $method_id, $instance_id, $rate_id = '' ) {
		$identifiers = array();
		...
		$identifiers[] = $instance_id ? $method_id . ':' . $instance_id : $method_id;
	...
	private function get_canonical_order_shipping_item_rate_ids( $order_shipping_items ) {
		...
		foreach ( $order_shipping_items as $order_shipping_item ) {
			$canonical_rate_ids = array_merge(
				$canonical_rate_ids,
				$this->get_rate_identifiers( $order_shipping_item->get_method_id(), $order_shipping_item->get_instance_id() )
			);
		}
		...
		if ( WC()->cart && is_callable( array( WC()->cart, 'get_shipping_methods' ) ) ) {
			foreach ( (array) WC()->cart->get_shipping_methods() as $rate ) {
				if ( is_object( $rate ) && is_callable( array( $rate, 'get_method_id' ) ) && is_callable( array( $rate, 'get_instance_id' ) ) ) {
					$canonical_rate_ids = array_merge(
						$canonical_rate_ids,
						$this->get_rate_identifiers(
							$rate->get_method_id(),
							$rate->get_instance_id(),
							is_callable( array( $rate, 'get_id' ) ) ? $rate->get_id() : ''
						)
					);
				}
			}
			...
			foreach ( $chosen_package_rate_ids as $package_key => $chosen_package_rate_id ) {
				if ( ! empty( $shipping_packages[ $package_key ]['rates'][ $chosen_package_rate_id ] ) ) {
					...
					$canonical_rate_ids = array_merge(
						$canonical_rate_ids,
						$this->get_rate_identifiers(
							$chosen_rate->get_method_id(),
							$chosen_rate->get_instance_id(),
							is_callable( array( $chosen_rate, 'get_id' ) ) ? $chosen_rate->get_id() : ''
						)
					);
				}
			}
